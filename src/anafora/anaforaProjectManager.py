...
class AnaforaProjectManager:
    ...
    def loadCorpus(ps, projectName):
        ...
        if AnaforaProjectManager.projectList[projectName] is None:
            ...
            AnaforaProjectManager.projectList[projectName] = sorted(
                [
                    cName
                    for cName in os.listdir(projectPath)
                    if os.path.isdir(os.path.join(projectPath, cName))
                    and cName[0] != "."
                ]
            )
    ...
    def checkExist(ps, projectName, corpusName="", taskName=""):
        ...
        if corpusName not in AnaforaProjectManager.projectList[projectName]:
            raise Exception(
                "Corpus '%s' is not exist under Project '%s'"
                % (corpusName, projectName)
            )
        ...
        if not os.path.isdir(
            ...
            raise Exception(
                ...
                % (taskName, projectName, corpusName)
            )
        return True
    ...
    def searchAllTask(
        ...
        command_str = (
            "find %s -maxdepth 2 -type f -name '*.%s%s.*.xml' | sed -r 's/^.*\/([^\/]+)\/[^\/]+.xml$/\\1/g' | sort -u"
    ...
    def getAllTask(ps, projectName, corpusName):
        ...
        command_str = (
            "find %s -mindepth 1 -maxdepth 1 -type d | sed -r 's/%s\/([^\/]+)$/\\1/g' | sort -u"
    ...
    def getAllSubTaskFromProjectCorpusTaskName(ps, projectName, corpusName, taskName):
        ...
        return sorted(
            [
                tName
                for tName in os.listdir(taskPath)
                if os.path.isdir(os.path.join(taskPath, tName))
                and tName[0] != "."
                and os.path.isfile(os.path.join(taskPath, tName, tName))
            ]
        )

    @staticmethod
    def searchAvailableCrossTask(
        ps,
        projectName,
        corpusName,
        annotator,
        ...
        """
        standardAvailableTasks = AnaforaProjectManager.searchAvailableTask(
            ps,
            projectName,
            corpusName,
            annotator,
            ...
            for newTaskName in standardAvailableTasks["n"]:
                subTaskList = (
                    AnaforaProjectManager.getAllSubTaskFromProjectCorpusTaskName(
                        ps, projectName, corpusName, newTaskName
                    )
                )
                taskPath = os.path.join(
                    settings.ANAFORA_PROJECT_FILE_ROOT,
                ...
                for subTaskName in subTaskList:
                    ...
                    for projectXMLFileName in [
                        pFileName
                        for pFileName in os.listdir(subTaskPath)
                        if os.path.isfile(os.path.join(subTaskPath, pFileName))
                        and pFileName[0] != "."
                        and pFileName[-4:] == ".xml"
                    ]:
                        try:
                        ...
                        if (
                            subTaskFile.taskName == subTaskName
                            and subTaskFile.schemaName == schemaName
                            and subTaskFile.modeName == modeName
                            and subTaskFile.isPreannotation
                        ):
    ...
    def searchAvailableTask(
        ...
        for taskName in AnaforaProjectManager.getAllTask(ps, projectName, corpusName):
            ...
            for projectXMLFileName in [
                pFileName
                for pFileName in os.listdir(taskPath)
                if os.path.isfile(os.path.join(taskPath, pFileName))
                and pFileName[0] != "."
                and pFileName[-4:] == ".xml"
            ]:
                try:
                    ...
                    continue
                if (
                    taskFile.taskName == taskName
                    and taskFile.schemaName == schemaName
                    and taskFile.modeName == modeName
                ):
    ...
    @staticmethod
    def searchAvailableAdjudicationTask(
        ps,
        projectName,
        corpusName,
        adjudicator,
    ...
    def getAnnotator(
        ...
        annotatorList = {}
        annotatorList["i"] = AnaforaProjectManager.getInprogressAnnotator(
            ps,
            projectName,
            corpusName,
            taskName,
        ...
        annotatorList["c"] = AnaforaProjectManager.getCompletedAnnotator(
            ps,
            projectName,
            corpusName,
            taskName,
    ...
    def getInprogressAnnotator(
        ...
        fileList = glob.glob(
            os.path.join(
                ...
                corpusName,
                taskName,
                "%s.%s%s.*.inprogress.xml"
                % (
                    taskName,
                    mode.getSchemaName(),
                    "-Adjudication" if isAdjudication else "",
                ),
            )
        )
        ...
        return sorted(
            [
                annotatorMatcher.group(1)
                for annotatorMatcher in [
