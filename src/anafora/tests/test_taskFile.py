...
from ..taskFile import TaskFile, ValidationError
...
class TaskFileTest(TestCase):
    ...
    def test_constructor(self):
        ...
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'). %s" % (fileName0, TaskFile.formatStr),
        )
        fileName1 = "ID017_clinic_049.Coreference.wech5560.completed"
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'). %s" % (fileName1, TaskFile.formatStr),
        )

        fileName2 = "ID017_clinic_049.Coreference.wech5560.complete.xml"
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'): please check the value of 'completed/inprogress'. %s"
            % (fileName2, TaskFile.formatStr),
        )
        fileName3 = "ID017_clinic_049.Coreference.wech5560.in-progress.xml"
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'): please check the value of 'completed/inprogress'. %s"
            % (fileName3, TaskFile.formatStr),
        )

        fileName4 = "ID017_clinic_049.Temporal-Entity-Adj.wech5560.complete.xml"
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'): please check the value of your adjudication indicator. %s"
            % (fileName4, TaskFile.formatStr),
        )

        fileName5 = (
            "ID017_clinic_049.Temporal-Entity-Adjudication-QQ.wech5560.complete.xml"
        )
        self.assertRaisesRegexp(
            ValidationError,
            "XML File format error ('%s'). %s" % (fileName5, TaskFile.formatStr),
        )
        ...
        fileName10 = (
            "ID017_clinic_049.Temporal-Entity-Adjudication.wech5560.completed.xml"
        )
