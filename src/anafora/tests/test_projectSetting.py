...
class ModeTests(TestCase):
    ...
    def test_parse(self):
        ...
        self.assertListEqual(
            mode2.schemaFile, ["temporal-schema.xml", "umls-schema.xml"]
        )
...
class SchemaTest(TestCase):
    def test_parse(self):
        ...
        self.assertListEqual(
            mode0_2.schemaFile, ["temporal-schema.xml", "umls-schema.xml"]
        )
        ...
        self.assertListEqual(
            mode2.schemaFile, ["propbank-schema.xml", "coref-schema.xml"]
        )
...
class ProjectTest(TestCase):
    def test_parse(self):
        ...
        self.assertListEqual(
            project0.allowedSchemas,
            ["Temporal", "Coreference", "Thyme2v1", "Medicine", "UMLS"],
        )
...
class ProjectSettingTests(TestCase):
    ...
    def test_getMode(self):
        ...
        ps.parseFromFile(self.settingFile)
        self.assertRaisesRegexp(
            Exception, "Get schema 'Temporal' error", ps.getMode, "Temporal"
        )
        self.assertRaisesRegexp(
            Exception,
        ...
        self.assertRaisesRegexp(
            Exception, "Get schema 'FakeTemporal' error", ps.getMode, "FakeTemporal"
        )
        self.assertRaisesRegexp(
            Exception,
            "Get schema 'FakeTemporal' error",
            ps.getMode,
            "FakeTemporal",
            None,
        )
        ...
        self.assertRaisesRegexp(
            Exception,
            "Get schema 'PropBank' with mode name 'fakeMode' error",
            ps.getMode,
            "PropBank",
            "fakeMode",
        )
    ...
    def test_getSchemaFileNameFromSchemaAndMode(self):
        ...
        self.assertRaisesRegexp(
            ...
            ps.getSchemaFileNameFromSchemaAndMode,
            "Temporal",
            1,
            "Entity",
        )
    ...
    def test_getSchema(self):
        ...
        ps.parseFromFile(self.settingFile)
        self.assertRaisesRegexp(
            Exception, "Get schema 'Temp' error", ps.getSchema, "Temp"
        )
        ...
        self.assertListEqual(
            sorted(schema2.modes), sorted(["Anatomy", "Correction", "Coreference"])
        )
    ...
    def test_getSchemaMap(self):
        ...
        self.assertListEqual(
            sorted(["Entity", "Relation", "RelationReGold"]),
        ...
        self.assertListEqual(
            sorted(["Coreference", "Correction", "Anatomy"]),
