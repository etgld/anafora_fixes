    ...
    def __init__(self, fileName):
        ...
        if len(fileName) <= 4 or fileName[-4:] != ".xml":
            raise ValidationError(
                "XML File format error ('%s'). %s" % (str(fileName), TaskFile.formatStr)
            )
        if fileName.count(".") <= 3:
            raise ValidationError(
                "XML File format error ('%s'), should be '$taskName.$schemaName.$annotator.(completed/inpgrogress).xml'"
                % (str(fileName))
            )
        ...
        if taskTerms[-2] == "completed":
            ...
            raise ValidationError(
                ...
                % (str(fileName), TaskFile.formatStr)
            )
