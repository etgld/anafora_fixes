...
js_anafora = [
    ...
    for js_file in [
        ...
        "propertyFrame.js",
        "annotateFrame.js",
        "aObjSelectionMenu.js",
        "projectSelector.js",
        "anaforaAdjudicationProject.js",
        "anaforaCrossProject.js",
        "anaforaCrossAdjudicationProject.js",
        "relationFrame.js",
        "stablePair.js",
        "eventLogging.js",
    ]
...
js_schemaSpecific = {
    "Coreference": {
        "adjudication": ["js/anafora/anaforaAdjudicationProjectCoreference.js"]
    }
}
...
def authenticate(
    ...
    if isView:
        if not request.META["REMOTE_ADMIN"]:
            return HttpResponseForbidden(
                "%s does not have the 'view' right" % request.META["REMOTE_USER"]
            )

    if projectName != "":
        if not os.path.isdir(os.path.join(AnaforaProjectManager.rootPath, projectName)):
            return HttpResponseForbidden(
                "Project Name: '%s' does not exists" % str(projectName)
            )

    if corpusName != "":
        if not os.path.isdir(
            os.path.join(AnaforaProjectManager.rootPath, projectName, corpusName)
        ):
            return HttpResponseForbidden(
                "Corpus Name: '%s' does not exists" % str(corpusName)
            )
    if taskName != "":
        if not os.path.isdir(
            os.path.join(
                AnaforaProjectManager.rootPath, projectName, corpusName, taskName
            )
        ):
            return HttpResponseForbidden(
                "Task Name: '%s' does not exists" % str(taskName)
            )
...
def selectCorpus(request, projectName, corpusName):
    ...
    contextContent = basicContextContent
    contextContent["settingVars"] = assignSettingVars(
        request, projectName=projectName, corpusName=corpusName
    )
    # contextContent.update(csrf(request))
    # context = Context(contextContent)
    return render(request, "anafora/index.html", contextContent)
    ...
    if projectName == "":
        ...
        try:
            if isCrossDoc:
                ...
                rawTextFileList = (
                    AnaforaProjectManager.getAllSubTaskFromProjectCorpusTaskName(
                        ps, projectName, corpusName, taskName
                    )
                )
                for rawTextFile, sTaskName in [
                    (
                        os.path.join(
                    ...
                    for sTaskName in rawTextFileList
                ]:
                    readRawText(rawTextFile, sTaskName)
            else:
                rawTextFile = os.path.join(
                    settings.ANAFORA_PROJECT_FILE_ROOT,
                    projectName,
                    corpusName,
                    taskName,
...
def getCompleteAnnotator(
    request, projectName, corpusName, taskName, schemaName, schemaMode=None, isAdj=None
):
    if not isSchemaExist(schemaName, schemaMode):
    ...
    if isAdjudicator(request):
        annotatorName = AnaforaProjectManager.getCompletedAnnotator(
            ps, projectName, corpusName, taskName, schemaName, schemaMode
        )
...
def getInprogressAnnotator(
    ...
    ps = getProjectSetting()
    if isAdjudicator(request):
        annotatorName = AnaforaProjectManager.getInprogressAnnotator(
            ps=ps,
            projectName=projectName,
            corpusName=corpusName,
            taskName=taskName,
...
def getAnnotator(
    ...
    if isAdjudicator(request):
        ...
        annotatorName = AnaforaProjectManager.getAnnotator(
            ps,
            projectName,
            corpusName,
            taskName,
            schemaName,
            schemaMode=schemaMode,
            isAdjudication=(isAdj == "Adjudication"),
        )
...
def getAnaforaXMLFile(
    ...
    if (
        annotatorName != ""
        and annotatorName != request.META["REMOTE_USER"]
        and isAdjudicator(request) is not True
    ...
    try:
        ...
        raise Exception(f"Open XML File {anaforaXMLFile} error")
...
def getSchema(request, schema, schemaIdx=-1):
    ...
    try:
        if "." in schema:
            ...
            mode = None
        (schemaFileName, moreSchema) = ps.getSchemaFileNameFromSchemaAndMode(
            schema, schemaIdx, mode
        )

        schemaFile = os.path.join(
            settings.ANAFORA_PROJECT_FILE_ROOT, ".schema", schemaFileName
        )
        fhd = open(schemaFile)
        schemaXML = fhd.read()
        fhd.close()
...
def getCrossTaskFromProjectCorpusName(
    request, projectName, corpusName, schemaName, modeName=None
):
    ...
    ps = getProjectSetting()
    try:
        taskName = AnaforaProjectManager.searchAvailableCrossTask(
            ps,
            projectName,
            corpusName,
            request.META["REMOTE_USER"],
...
def getAllTask(
    ...
    ps = getProjectSetting()
    if isAdjudicator(request):
        taskName = AnaforaProjectManager.searchAllTask(
            ps,
            projectName,
            corpusName,
            schemaName,
            schemaMode=schemaMode,
            isAdj=(isAdj == "Adjudication"),
            isCrossDoc=(crossDoc == "_crossDoc"),
        )
        return HttpResponse(json.dumps(taskName))
    else:
        return HttpResponseForbidden("access not allowed")
...
def getAdjudicationTaskFromProjectCorpusName(
    request, projectName, corpusName, schemaName, schemaMode=None, crossDoc=None
):
    if request.method != "GET":
        return HttpResponseForbidden()
    ...
    ps = getProjectSetting()
    if isAdjudicator(request):
        taskName = AnaforaProjectManager.searchAvailableAdjudicationTask(
            ps,
            projectName,
            corpusName,
            request.META["REMOTE_USER"],
...
# Doesn't get called so put a pin in it
...
def getSubTaskFromProjectCorpusTaskName(
    request, projectName, corpusName, parentTaskName
):
    ...
    ps = getProjectSetting()
    try:
        subTaskName = AnaforaProjectManager.getAllSubTaskFromProjectCorpusTaskName(
            ps, projectName, corpusName, parentTaskName
        )
    except Exception as e:
        return HttpResponseNotFound(str(e))
...
def getTaskFromProjectCorpusName(
    request, projectName, corpusName, schemaName, schemaMode=None, crossDoc=False
):
    ps = getProjectSetting()
    try:
        if crossDoc:
            return getCrossTaskFromProjectCorpusName(
                request, projectName, corpusName, schemaName, schemaMode
            )
            # taskName = AnaforaProjectManager.searchAvailableCrossTask(projectName, corpusName, schemaName, request.META["REMOTE_USER"], ps)
        else:
            taskName = AnaforaProjectManager.searchAvailableTask(
                ps,
                projectName,
                corpusName,
                request.META["REMOTE_USER"],
...
def writeFile(
    ...
    with open(fileName, mode="w+", encoding="utf-8") as fhd:
...
def setCompleted(
    ...
    if os.path.exists("%s.inprogress.xml" % fileName):
        subprocess.call(
            ["mv", "%s.inprogress.xml" % fileName, "%s.completed.xml" % fileName]
        )
        subprocess.call(
        ...
        if isAdj is not None or mode.directSetGold:
            ...
            if mode.directSetGold:
                subprocess.call(
                    "sed -u -i 's/@%s/@gold/' %s.completed.xml"
            ...
            for tModeName in schema.modes:
                ...
                if (
                    tMode.needPreannotation
                    and tMode.preannotationFromMode is not None
                    and tMode.preannotationFromMode.name == mode.name
                ):
                    fileNamePreannotation = (
                        filePath
                        + "/"
...
def isAdjudicator(request):
    ...
    if "REMOTE_ADMIN" in request.META:
        ...
        return grpID in [
            g.gr_gid for g in grp.getgrall() if testAdjudicator in g.gr_mem
        ]
    ...
    if projectSetting is None:
        ...
        if not os.path.isfile(parseFile):
            ...
            raise ImproperlyConfigured(
                "Error loading ANAFORA_PROJECT_SETTING_FILENAME in web/settings.py file. Please check the value of ANAFORA_PROJECT_FILE_ROOT, ANAFORA_PROJECT_SETTING_FILENAME accordingly"
            )
