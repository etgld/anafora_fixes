...
class ProjectSetting(object):
    ...
    def parseFromFile(self, filePath):
        ...
        for childNode in [
            tNode for tNode in psDOM.childNodes if tNode.nodeType == tNode.ELEMENT_NODE
        ]:
            if childNode.tagName == "projects":
                for projectNode in [
                    tNode
                    for tNode in childNode.childNodes
                    if tNode.nodeType == tNode.ELEMENT_NODE
                ]:
                    project = Project.parseFromXMLDOM(projectNode)
                    if os.path.isdir(
                        os.path.join(settings.ANAFORA_PROJECT_FILE_ROOT, project.name)
                    ):
            ...
            for modeName in schema.modes:
                ...
                if mode.needPreannotation:
                    ...
                    if mode.preannotationFromMode.strip() == "":
                        ...
                        mode.preannotationFromMode = schema.getMode(
                            mode.preannotationFromMode
                        )
    ...
    def getSchemaFileNameFromSchemaAndMode(
        self, schemaName, schemaFileIdx=0, modeName=None
    ):
        """
        @type schemaName:	   str
...
class Schema(object):
    ...
    def parseFromXMLDOM(cls, schemaNode):
        ...
        if name == "":
            raise Exception(
                "'name' attribute is not exist in mode xml" + schemaNode.toxml()
            )
        ...
        for childNode in [
            tNode
            for tNode in schemaNode.childNodes
            if tNode.nodeType == tNode.ELEMENT_NODE
        ]:
    ...
    def getMode(self, modeName=None):
        ...
        if modeName not in self.modes:
            if modeName == None:
                ...
                raise Exception(
                    "Get schema '%s' with mode name '%s' error"
                    % (self.name, str(modeName))
                )
...
class Mode(object):
    ...
    def parseFromXMLDOM(cls, modeNode, schemaName):
        ...
        if name == "":
            raise Exception(
                "'name' attribute is not exist in mode xml" + modeNode.toxml()
            )
        ...
        for childNode in [
            tNode
            for tNode in modeNode.childNodes
            if tNode.nodeType == tNode.ELEMENT_NODE
        ]:
...
class Project(object):
    def __init__(
        self, name, admins, numOfAnnotator=2, annotators=[], allowedSchemas=[]
    ):
        """
        @type name:		 str
    ...
    def parseFromXMLDOM(cls, projectNode):
        ...
        if name == "":
            raise Exception(
                "'name' attribute not exist in project xml " + projectNode.toxml()
            )
        ...
        for childNode in [
            tNode
            for tNode in projectNode.childNodes
            if tNode.nodeType == tNode.ELEMENT_NODE
        ]:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            'Please validate the XML file with "python projectSetting.py pathToSettingFile"'
        )
        exit()
